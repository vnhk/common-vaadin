...
@Service
public class LogEntityApplicationNameLoader implements DynamicConfigLoader {

    private final LogService logService;

    public LogEntityApplicationNameLoader(LogService logService) {
    ...
    public @NotNull List<String> getDynamicStrValuesMap() {
        return logService.loadAppsName().stream().filter(s -> s != null && !s.isBlank()).toList();
    }
}
